// ============================================================
// TRANSLATIONS - Teik Granite Quarry Website
// Languages: English (en), Malay (ms), Chinese (zh)
// ============================================================

const TRANSLATIONS = {
  // ============================================================
  // ENGLISH
  // ============================================================
  en: {
    // ---- NAVIGATION ----
    nav: {
      company: "Company",
      about: "About",
      products: "Products",
      facilities: "Facilities",
      contact: "Contact"
    },
    logo: {
      company: "TEIK GRANITE QUARRY",
      reg: "196001000153 (3821-P)"
    },

    // ---- LANGUAGE NAMES ----
    lang: {
      en: "🇬🇧 EN",
      ms: "🇲🇾 BM",
      zh: "🇨🇳 CN"
    },

    // ---- COMMON ----
    common: {
      learnMore: "Learn More →",
      requestQuote: "Request Quote",
      viewMap: "View Locations on Map",
      backToTop: "Back to top",
      sendMessage: "Send Message",
      copyright: "© 2026 Teik Granite Quarry Sdn Bhd. All Rights Reserved.",
      established: "Established: 20 June 1960",
      regNo: "Registration No: 196001000153 (3821-P)",
      tagline: "A trusted Malaysian quarry company supplying premium granite and construction materials for over 60 years.",
      quickLinks: "Quick Links",
      groupCompanies: "Group Companies",
      connectWithUs: "Connect With Us",
      phone: "04-261 4477",
      fax: "04-262 2818",
      location: "Pulau Tikus, Penang, Malaysia",
      watchVideo: "Watch Video"
    },

    // ---- FOOTER GROUP COMPANIES ----
    footer: {
      groupCompanies: {
        teikGraniteQuarry: "Teik Granite Quarry Sdn Bhd",
        teikGraniteCorporation: "Teik Granite Corporation Sdn Bhd",
        tgqTanjungBungah: "TGQ (Tanjung Bungah) Sdn Bhd",
        tgqAirItam: "TGQ (Air Itam) Sdn Bhd"
      }
    },

    // ---- INDEX PAGE ----
    index: {
      hero: {
        badge: "SINCE 1960",
        title: 'Building Malaysia <br /><span class="highlight-red">Since 1960</span>',
        subtitle: '<strong>Trusted Quarry Solutions for Infrastructure & Construction</strong><br />Established in 1960, Teik Granite Quarry Sdn. Bhd. has over six decades of experience supplying premium granite aggregates, manufactured sand and quarry materials throughout Penang and Malaysia.'
      },
      overview: {
        badge: "Since 1960",
        title: "Company Overview",
        subtitle: "Building Malaysia's infrastructure with quality granite",
        p1: 'Established in 1960, <span class="highlight">Teik Granite Quarry Sdn. Bhd.</span> is one of Penang\'s established quarry operators.',
        p2: "For more than 60 years, we have supplied quality granite products for highways, bridges, commercial developments, residential projects, factories, ports and infrastructure works.",
        p3: "Today, we continue to combine decades of quarrying experience with modern production methods to provide reliable building materials for Malaysia's construction industry.",
        stats: {
          years: "Years of Excellence",
          companies: "Group Companies",
          incorporated: "Year Incorporated",
          location: "Location"
        }
      },
      subsidiaries: {
        title: "Group of Companies",
        subtitle: "Teik Granite Group comprises the following entities",
        headOffice: "Head Office",
        corporateOffice: "Corporate Office",
        quarrySite: "Quarry Site",
        address: "Address",
        email: "Email",
        phone: "Phone",
        fax: "Fax",
        website: "Website",
        site: "Site"
      },
      products: {
        title: "Our Products",
        subtitle: "Premium quarry materials for construction and infrastructure",
        crushedStone: "Crushed Stone",
        gravel: "Gravel (Natural)",
        sand: "Sand (Manufactured / M-Sand)",
        recycled: "Recycled Aggregate",
        crushedStoneSpec: '3/4" & 1 1/2" & 3/8"',
        gravelSpec: "Washed & Screened",
        sandSpec: "M.Sand / Quarry Dust",
        recycledSpec: "Eco-Friendly",
        crushedStoneDesc: "High-quality crushed granite for concrete production, asphalt paving, and road construction.",
        gravelDesc: "Natural gravel for drainage systems, landscaping, pathways, and decorative applications.",
        sandDesc: "Manufactured sand for construction, plastering, backfilling, and building material production.",
        recycledDesc: "Sustainable recycled aggregate for sub-base, road construction, and environmentally conscious projects.",
        sirim: "SIRIM Certified",
        premium: "Premium Quality",
        infrastructure: "Infrastructure Grade",
        eco: "Eco-Friendly",
        industries: {
          title: "Suitable Industries",
          subtitle: "Our products are widely used in:",
          highway: "Highway Construction",
          road: "Road Works",
          bridge: "Bridge",
          commercial: "Commercial Buildings",
          residential: "Residential Housing",
          industrial: "Industrial Factories",
          infrastructure: "Infrastructure Projects",
          reclamation: "Land Reclamation",
          drainage: "Drainage Works",
          landscaping: "Landscaping",
          readyMix: "Ready-Mix Concrete Plants",
          asphalt: "Asphalt Plants",
          civil: "Civil Engineering Projects"
        },
        showMore: "Show Product Details"
      },
      process: {
        title: "From Rock to Reliable Construction Materials",
        subtitle: "Our quarry process ensures quality at every step",
        step1: "Drill & Blast",
        step1Desc: "Controlled blasting to extract quality granite",
        step2: "Excavate",
        step2Desc: "Hydraulic excavators remove blasted rock safely and efficiently",
        step3: "Crushing Plant",
        step3Desc: "Primary and secondary crushing produce different aggregate sizes",
        step4: "Load & Haul",
        step4Desc: "Heavy-duty dump trucks transport granite to the stockpile"
      },
      video: {
        title: "Our Quarry Operations",
        subtitle: "Watch our granite extraction and blasting process in action"
      }
    },

    // ---- ABOUT PAGE ----
    about: {
      hero: {
        title: "About Us",
        subtitle: "Building Malaysia's infrastructure with quality granite since 1960"
      },
      history: {
        title: "Company History",
        p1: "Teik Granite Quarry (TGQ) is a pioneer in the quarry industry in the state of Penang. TGQ was founded in the early 1950's by the family's patriarch, Mr. Loh Poh Heng when he first operated a quarry at Air Itam at the foothill of Penang Hill.",
        p2: "During the 1960's, the quarry business was expanded to Tanjung Bungah, then Bayan Lepas in the Island and Penanti, Bukit Mertajam in the mainland. During the peak, there were four quarries operating in Penang. However, the Bayan Lepas quarry which was operated as a project quarry lasted for a few years only. The Air Itam quarry too ceased operation around 1992, a decision taken on a voluntary basis.",
        p3: "To date the Tanjong Bungah and Penanti quarries are still active and remain operating as commercial quarries both in the Island and mainland.",
        timeline: {
          title: "Time Line",
          location: "Location of Quarry",
          status: "Status",
          airItam: "Air Itam, Penang",
          bayanLepas: "Bayan Lepas, Penang",
          tanjungBungah: "Tanjung Bungah, Penang",
          penanti: "Penanti, Bukit Mertajam",
          ceased: "Ceased",
          active: "Active"
        }
      },
      projects: {
        title: "Participation in Major Projects in Penang",
        subtitle: "Throughout the year of operation, TGQ has participated in many major infrastructures and building projects, both as a material supplier and also as a contractor",
        airport: {
          title: "Penang International Airport",
          desc: "Worked with Maeda Construction Co. as a subcontractor and also as the supplier of all rock materials.",
          company: "Maeda Construction Co."
        },
        bridge: {
          title: "Penang Bridge 1st & Lim Chong Eu Expressway",
          desc: "Worked with Hyundai Engineering and Construction Company as a supplier of rock materials and as a road constructor for a portion of the Lim Chong Eu Expressway.",
          company: "Hyundai Engineering & Construction"
        },
        komtar: {
          title: "Komtar Tower",
          desc: "Worked with Lim Kah Ngam Malaysia as their main supplier of aggregates for their concrete requirement for all building works.",
          company: "Lim Kah Ngam Malaysia"
        },
        skyscrapers: {
          title: "Major Skyscrapers in Penang Island",
          desc: "Major supplier of aggregates to contractors and readymixed concrete producers for numerous high rise projects throughout Penang Island.",
          company: "Various Contractors"
        },
        northSouth: {
          title: "North-South Expressway",
          desc: "Supplier of road-based rock materials and aggregates for concrete plants to United Engineers contractors for the northern region.",
          company: "United Engineers"
        },
        container: {
          title: "North Butterworth Container Terminal",
          desc: "Supplier of armour rocks & stones to IJM Construction for the port's seashore protection.",
          company: "IJM Construction"
        },
        secondBridge: {
          title: "Penang 2nd Bridge",
          desc: "Worked with China Harbour Engineering Co. as a supplier of quality aggregates for their high grade concrete production.",
          company: "China Harbour Engineering Co."
        },
        doubleTrack: {
          title: "Electrified Double Track Project",
          desc: "Supplier of ballast stones for railway tracks and stone materials for road base and concrete requirements.",
          company: "Various Contractors"
        }
      },
      historical: {
        title: "Historical Quarry Sites",
        subtitle: "Preserving the rich heritage of Penang's quarrying industry",
        airItam: {
          title: "Air Itam Quarry",
          subtitle: "Where it all began...",
          ceased: "Ceased",
          caption1: "1957 painting by Yong Mun Sen - Air Itam Quarry operations",
          caption2: "Old concrete stone bunkers with rail tracks for transportation",
          text1: "1957 painting by well known artist Yong Mun Sen of Air Itam Quarry operations in the early days (1950's) where use of jack hammers and rock breaking by manual labour using hammers were the norm.",
          text2: "Air Itam Quarry's old concrete stone bunkers. Stones/rocks broken down to suitable sizes were then transported to the bunkers by wheel buckets on rail tracks for storage and subsequent sales."
        },
        tanjungBungah: {
          title: "Tanjong Bungah Quarry",
          subtitle: "Active commercial quarry",
          active: "Active",
          caption1: "Aerial photograph of Tanjong Bungah Quarry area",
          caption2: "Aerial view of the quarry operations"
        },
        penanti: {
          title: "Penanti Quarry",
          subtitle: "Active commercial quarry",
          active: "Active",
          caption1: "Aerial photograph of Penanti Quarry area",
          caption2: "Aerial view of the quarry operations"
        }
      },
      certificate: {
        title: "Certificate Awards",
        subtitle: "Recognized for excellence in quarry operations and safety standards",
        year2021: "Certificate of Achievement",
        year2022: "Certificate of Achievement",
        year2023: "Certificate of Achievement",
        year2024: "Certificate of Achievement"
      },
      esg: {
        title: "Our Commitment to Sustainability",
        subtitle: "Building a greener future through responsible quarrying",
        environmental: {
          title: "Environmental",
          desc: "We are committed to sustainable quarrying practices, including responsible land rehabilitation, dust suppression, and water conservation to minimize our environmental footprint.",
          tags: ["Climate Action", "Life on Land"]
        },
        safety: {
          title: "Safety",
          desc: "Comprehensive safety protocols and regular training ensure the well-being of our workforce and compliance with the highest industry safety standards.",
          tags: ["Good Health", "Decent Work"]
        },
        community: {
          title: "Community",
          desc: "We actively engage with local communities, supporting economic development and creating employment opportunities in the regions where we operate.",
          tags: ["No Poverty", "Decent Work", "Sustainable Cities"]
        }
      }
    },

    // ---- PRODUCTS PAGE ----
    products: {
      hero: {
        title: "Our Products",
        subtitle: "Premium granite and quarry materials for construction and infrastructure"
      },
      list: {
        aggregate34: {
          name: '3/4" AGGREGATE',
          spec: "Size: 20mm",
          desc: "High-quality crushed granite aggregate for concrete production, asphalt paving, and road construction. SIRIM certified.",
          tag: "SIRIM Certified"
        },
        aggregate112: {
          name: '1 1/2" AGGREGATE',
          spec: "Size: 40mm",
          desc: "Premium granite aggregate for heavy-duty concrete works, large-scale construction, and foundation projects.",
          tag: "SIRIM Certified"
        },
        msand: {
          name: "M.SAND (QUARRY DUST)",
          spec: "Fine Powder",
          desc: "Manufactured sand and quarry dust for construction, backfilling, plastering, and building material production.",
          tag: "Premium Quality"
        },
        crusherRun: {
          name: '2" CRUSHER RUN',
          spec: "Size: 50mm",
          desc: "Granite crusher run material for road base, sub-base, foundation works, and civil construction projects.",
          tag: "Infrastructure Grade"
        },
        block: {
          name: '3" X 4" BLOCK',
          spec: "Size: 75mm x 100mm",
          desc: "Cut-to-size granite blocks for architectural cladding, landscaping, retaining walls, and decorative stone applications.",
          tag: "Custom Sizes"
        },
        chipping: {
          name: '3/8" CHIPPING',
          spec: "Size: 10mm",
          desc: "Small granite chippings for landscaping, decorative pathways, drainage systems, and concrete finishing applications.",
          tag: "Premium Quality"
        }
      }
    },

    // ---- FACILITIES PAGE ----
    facilities: {
      hero: {
        title: "Our Facilities",
        subtitle: "State-of-the-art quarry and processing facilities"
      },
      list: {
        quarry: {
          title: "Granite Quarry",
          desc: "Our main quarry operation spans over decades of granite extraction. Equipped with modern drilling and blasting technology."
        },
        crushing: {
          title: "Crushing Plant",
          desc: "State-of-the-art crushing and screening plant producing high-quality aggregates of various sizes."
        },
        storage: {
          title: "Storage Facilities",
          desc: "Large covered storage areas for finished products, ensuring materials remain dry and ready for dispatch."
        },
        transport: {
          title: "Transport Fleet",
          desc: "Our own fleet of heavy-duty haulers and trucks ensures timely and reliable delivery across Malaysia."
        },
        safety: {
          title: "Safety & Environment",
          desc: "Committed to sustainable quarrying practices with comprehensive safety and environmental management."
        }
      }
    },

    // ---- CONTACT PAGE ----
    contact: {
      hero: {
        title: "Contact Us",
        subtitle: "Get in touch with Teik Granite Quarry for all your construction material needs"
      },
      form: {
        title: "Send an Enquiry",
        subtitle: "Fill out the form and our team will respond within 24 hours.",
        name: "Full Name",
        namePlaceholder: "Enter your full name",
        nameError: "Please enter your full name",
        email: "Email Address",
        emailPlaceholder: "you@example.com",
        emailError: "Please enter a valid email address",
        phone: "Phone Number",
        phonePlaceholder: "012-345 6789",
        phoneError: "Please enter your phone number",
        subject: "Subject",
        subjectPlaceholder: "Select a subject...",
        subjectError: "Please select a subject",
        subjectOptions: {
          general: "General Enquiry",
          quotation: "Product Quotation",
          bulk: "Bulk Order",
          logistics: "Logistics & Delivery",
          partnership: "Partnership",
          other: "Other"
        },
        message: "Message",
        messagePlaceholder: "Tell us about your project...",
        messageError: "Please enter your message",
        sending: "Sending...",
        send: "Send Message",
        success: {
          title: "Success!",
          message: "Your message has been sent successfully! We will get back to you within 24 hours."
        },
        error: {
          title: "Invalid!",
          message: "Please fill in all required fields correctly.",
          tryAgain: "Something went wrong. Please try again or contact us directly via phone."
        }
      },
      info: {
        headOffice: "Head Office",
        corporateOffice: "Corporate Office",
        headOfficeCompany: "Teik Granite Quarry Sdn Bhd",
        corporateOfficeCompany: "Teik Granite Corporation Sdn Bhd",
        contactNumbers: "Contact Numbers",
        companyInfo: "Company Information",
        email: "Email",
        main: "Main",
        corporate: "Corporate"
      },
      map: {
        title: "Find Us",
        subtitle: "Visit our offices and quarry sites across Penang",
        pulauTikus: {
          name: "Teik Granite Quarry Sdn Bhd / Teik Granite Corporation Sdn Bhd",
          location: "Pulau Tikus, Penang",
          badge: "Head Office / Corporate Office",
          address: "368-2-15, Level 2, Bellisa Row, Jalan Burmah, 10350 Penang"
        },
        tanjungBungah: {
          name: "TGQ (Tanjung Bungah) Sdn Bhd",
          location: "Tanjung Bungah, Penang",
          badge: "Quarry Site",
          address: "Tanjung Bungah, 11200 Penang"
        },
        penanti: {
          name: "TGQ (Air Itam) Sdn Bhd",
          location: "Penanti, Penang",
          badge: "Quarry Site",
          address: "Penanti Quarry, Kubang Semang, Penang"
        }
      }
    }
  },

  // ============================================================
  // MALAY (Bahasa Malaysia)
  // ============================================================
  ms: {
    // ---- NAVIGATION ----
    nav: {
      company: "Syarikat",
      about: "Tentang",
      products: "Produk",
      facilities: "Kemudahan",
      contact: "Hubungi"
    },
    logo: {
      company: "TEIK GRANITE QUARRY",
      reg: "196001000153 (3821-P)"
    },

    // ---- LANGUAGE NAMES ----
    lang: {
      en: "🇬🇧 EN",
      ms: "🇲🇾 BM",
      zh: "🇨🇳 CN"
    },

    // ---- COMMON ----
    common: {
      learnMore: "Ketahui Lebih Lanjut →",
      requestQuote: "Minta Sebut Harga",
      viewMap: "Lihat Lokasi di Peta",
      backToTop: "Kembali ke atas",
      sendMessage: "Hantar Mesej",
      copyright: "© 2026 Teik Granite Quarry Sdn Bhd. Hak Cipta Terpelihara.",
      established: "Ditubuhkan: 20 Jun 1960",
      regNo: "No. Pendaftaran: 196001000153 (3821-P)",
      tagline: "Syarikat kuari Malaysia yang dipercayai membekalkan granit premium dan bahan binaan selama lebih 60 tahun.",
      quickLinks: "Pautan Pantas",
      groupCompanies: "Kumpulan Syarikat",
      connectWithUs: "Hubungi Kami",
      phone: "04-261 4477",
      fax: "04-262 2818",
      location: "Pulau Tikus, Pulau Pinang, Malaysia",
      watchVideo: "Tonton Video"
    },

    // ---- FOOTER GROUP COMPANIES ----
    footer: {
      groupCompanies: {
        teikGraniteQuarry: "Teik Granite Quarry Sdn Bhd",
        teikGraniteCorporation: "Teik Granite Corporation Sdn Bhd",
        tgqTanjungBungah: "TGQ (Tanjung Bungah) Sdn Bhd",
        tgqAirItam: "TGQ (Air Itam) Sdn Bhd"
      }
    },

    // ---- INDEX PAGE ----
    index: {
      hero: {
        badge: "SEJAK 1960",
        title: 'Membina Malaysia <br /><span class="highlight-red">Sejak 1960</span>',
        subtitle: '<strong>Penyelesaian Kuari Dipercayai untuk Infrastruktur & Pembinaan</strong><br />Ditubuhkan pada tahun 1960, Teik Granite Quarry Sdn. Bhd. mempunyai pengalaman lebih enam dekad membekalkan agregat granit premium, pasir buatan dan bahan kuari di seluruh Pulau Pinang dan Malaysia.'
      },
      overview: {
        badge: "Sejak 1960",
        title: "Gambaran Syarikat",
        subtitle: "Membina infrastruktur Malaysia dengan granit berkualiti",
        p1: 'Ditubuhkan pada tahun 1960, <span class="highlight">Teik Granite Quarry Sdn. Bhd.</span> adalah salah satu pengendali kuari yang mapan di Pulau Pinang.',
        p2: "Selama lebih 60 tahun, kami telah membekalkan produk granit berkualiti untuk lebuh raya, jambatan, pembangunan komersial, projek perumahan, kilang, pelabuhan dan kerja infrastruktur.",
        p3: "Hari ini, kami terus menggabungkan pengalaman kuari selama beberapa dekad dengan kaedah pengeluaran moden untuk menyediakan bahan binaan yang boleh dipercayai untuk industri pembinaan Malaysia.",
        stats: {
          years: "Tahun Kecemerlangan",
          companies: "Syarikat Kumpulan",
          incorporated: "Tahun Ditubuhkan",
          location: "Lokasi"
        }
      },
      subsidiaries: {
        title: "Kumpulan Syarikat",
        subtitle: "Kumpulan Teik Granite terdiri daripada entiti berikut",
        headOffice: "Pejabat Utama",
        corporateOffice: "Pejabat Korporat",
        quarrySite: "Tapak Kuari",
        address: "Alamat",
        email: "E-mel",
        phone: "Telefon",
        fax: "Faks",
        website: "Laman Web",
        site: "Tapak"
      },
      products: {
        title: "Produk Kami",
        subtitle: "Bahan kuari premium untuk pembinaan dan infrastruktur",
        crushedStone: "Batu Hancur",
        gravel: "Batu Kelikir (Semula Jadi)",
        sand: "Pasir (Buatan / M-Pasir)",
        recycled: "Agregat Kitar Semula",
        crushedStoneSpec: '3/4" & 1 1/2" & 3/8"',
        gravelSpec: "Dibasuh & Ditapis",
        sandSpec: "M.Pasir / Debu Kuari",
        recycledSpec: "Mesra Alam",
        crushedStoneDesc: "Granit hancur berkualiti tinggi untuk pengeluaran konkrit, turapan asfalt, dan pembinaan jalan raya.",
        gravelDesc: "Kelikir semula jadi untuk sistem saliran, landskap, laluan pejalan kaki, dan aplikasi hiasan.",
        sandDesc: "Pasir buatan untuk pembinaan, plasteran, penimbusan, dan pengeluaran bahan binaan.",
        recycledDesc: "Agregat kitar semula yang mampan untuk sub-asas, pembinaan jalan raya, dan projek yang mementingkan alam sekitar.",
        sirim: "Diperakui SIRIM",
        premium: "Kualiti Premium",
        infrastructure: "Gred Infrastruktur",
        eco: "Mesra Alam",
        industries: {
          title: "Industri Sesuai",
          subtitle: "Produk kami digunakan secara meluas dalam:",
          highway: "Pembinaan Lebuh Raya",
          road: "Kerja Jalan Raya",
          bridge: "Jambatan",
          commercial: "Bangunan Komersial",
          residential: "Perumahan Kediaman",
          industrial: "Kilang Perindustrian",
          infrastructure: "Projek Infrastruktur",
          reclamation: "Penambakan Tanah",
          drainage: "Kerja Saliran",
          landscaping: "Landskap",
          readyMix: "Loji Konkrit Sedia Campur",
          asphalt: "Loji Asfalt",
          civil: "Projek Kejuruteraan Awam"
        },
        showMore: "Lihat Butiran Produk"
      },
      process: {
        title: "Dari Batu ke Bahan Binaan yang Boleh Dipercayai",
        subtitle: "Proses kuari kami memastikan kualiti pada setiap langkah",
        step1: "Gerudi & Letupan",
        step1Desc: "Letupan terkawal untuk mengekstrak granit berkualiti",
        step2: "Penggalian",
        step2Desc: "Jentera penggali hidraulik mengeluarkan batu letupan dengan selamat dan cekap",
        step3: "Loji Penghancuran",
        step3Desc: "Penghancuran primer dan sekunder menghasilkan pelbagai saiz agregat",
        step4: "Muat & Angkut",
        step4Desc: "Lori dump berat mengangkut granit ke timbunan stok"
      },
      video: {
        title: "Operasi Kuari Kami",
        subtitle: "Saksikan proses pengekstrakan dan letupan granit kami"
      }
    },

    // ---- ABOUT PAGE ----
    about: {
      hero: {
        title: "Tentang Kami",
        subtitle: "Membina infrastruktur Malaysia dengan granit berkualiti sejak 1960"
      },
      history: {
        title: "Sejarah Syarikat",
        p1: "Teik Granite Quarry (TGQ) adalah perintis dalam industri kuari di negeri Pulau Pinang. TGQ ditubuhkan pada awal 1950-an oleh patriark keluarga, Encik Loh Poh Heng apabila beliau mula mengendalikan kuari di Air Itam di kaki Bukit Pulau Pinang.",
        p2: "Pada tahun 1960-an, perniagaan kuari dikembangkan ke Tanjung Bungah, kemudian Bayan Lepas di Pulau dan Penanti, Bukit Mertajam di tanah besar. Pada kemuncaknya, terdapat empat kuari beroperasi di Pulau Pinang. Walau bagaimanapun, kuari Bayan Lepas yang dikendalikan sebagai kuari projek hanya bertahan beberapa tahun sahaja. Kuari Air Itam juga berhenti operasi sekitar tahun 1992, keputusan yang diambil secara sukarela.",
        p3: "Sehingga kini kuari Tanjong Bungah dan Penanti masih aktif dan terus beroperasi sebagai kuari komersial di Pulau dan tanah besar.",
        timeline: {
          title: "Garis Masa",
          location: "Lokasi Kuari",
          status: "Status",
          airItam: "Air Itam, Pulau Pinang",
          bayanLepas: "Bayan Lepas, Pulau Pinang",
          tanjungBungah: "Tanjung Bungah, Pulau Pinang",
          penanti: "Penanti, Bukit Mertajam",
          ceased: "Berhenti",
          active: "Aktif"
        }
      },
      projects: {
        title: "Penglibatan dalam Projek Utama di Pulau Pinang",
        subtitle: "Sepanjang tahun operasi, TGQ telah menyertai banyak infrastruktur utama dan projek pembinaan, sebagai pembekal bahan dan juga sebagai kontraktor",
        airport: {
          title: "Lapangan Terbang Antarabangsa Pulau Pinang",
          desc: "Bekerja dengan Maeda Construction Co. sebagai subkontraktor dan juga sebagai pembekal semua bahan batuan.",
          company: "Maeda Construction Co."
        },
        bridge: {
          title: "Jambatan Pulau Pinang Pertama & Lebuhraya Lim Chong Eu",
          desc: "Bekerja dengan Hyundai Engineering and Construction Company sebagai pembekal bahan batuan dan sebagai pembina jalan untuk sebahagian Lebuhraya Lim Chong Eu.",
          company: "Hyundai Engineering & Construction"
        },
        komtar: {
          title: "Menara Komtar",
          desc: "Bekerja dengan Lim Kah Ngam Malaysia sebagai pembekal utama agregat untuk keperluan konkrit mereka untuk semua kerja bangunan.",
          company: "Lim Kah Ngam Malaysia"
        },
        skyscrapers: {
          title: "Pencakar Langit Utama di Pulau Pinang",
          desc: "Pembekal utama agregat kepada kontraktor dan pengeluar konkrit sedia campur untuk pelbagai projek tinggi di seluruh Pulau Pinang.",
          company: "Pelbagai Kontraktor"
        },
        northSouth: {
          title: "Lebuhraya Utara-Selatan",
          desc: "Pembekal bahan batuan berasaskan jalan dan agregat untuk loji konkrit kepada kontraktor United Engineers untuk wilayah utara.",
          company: "United Engineers"
        },
        container: {
          title: "Terminal Kontena Butterworth Utara",
          desc: "Pembekal batu perisai & batu kepada IJM Construction untuk perlindungan pantai pelabuhan.",
          company: "IJM Construction"
        },
        secondBridge: {
          title: "Jambatan Pulau Pinang Kedua",
          desc: "Bekerja dengan China Harbour Engineering Co. sebagai pembekal agregat berkualiti untuk pengeluaran konkrit gred tinggi mereka.",
          company: "China Harbour Engineering Co."
        },
        doubleTrack: {
          title: "Projek Landasan Berkembar Elektrik",
          desc: "Pembekal batu balast untuk landasan kereta api dan bahan batu untuk keperluan asas jalan dan konkrit.",
          company: "Pelbagai Kontraktor"
        }
      },
      historical: {
        title: "Tapak Kuari Bersejarah",
        subtitle: "Memelihara warisan industri kuari Pulau Pinang",
        airItam: {
          title: "Kuari Air Itam",
          subtitle: "Di mana semuanya bermula...",
          ceased: "Berhenti",
          caption1: "Lukisan 1957 oleh Yong Mun Sen - operasi Kuari Air Itam",
          caption2: "Bunker batu konkrit lama dengan landasan kereta api untuk pengangkutan",
          text1: "Lukisan 1957 oleh artis terkenal Yong Mun Sen tentang operasi Kuari Air Itam pada zaman awal (1950-an) di mana penggunaan tukul besi dan pemecahan batu secara manual menggunakan tukul adalah perkara biasa.",
          text2: "Bunker batu konkrit lama Kuari Air Itam. Batu/batu yang dipecahkan kepada saiz yang sesuai kemudian diangkut ke bunker menggunakan baldi roda di landasan kereta api untuk penyimpanan dan jualan seterusnya."
        },
        tanjungBungah: {
          title: "Kuari Tanjong Bungah",
          subtitle: "Kuari komersial aktif",
          active: "Aktif",
          caption1: "Fotografi udara kawasan Kuari Tanjong Bungah",
          caption2: "Pandangan udara operasi kuari"
        },
        penanti: {
          title: "Kuari Penanti",
          subtitle: "Kuari komersial aktif",
          active: "Aktif",
          caption1: "Fotografi udara kawasan Kuari Penanti",
          caption2: "Pandangan udara operasi kuari"
        }
      },
      certificate: {
        title: "Anugerah Sijil",
        subtitle: "Diiktiraf atas kecemerlangan dalam operasi kuari dan piawaian keselamatan",
        year2021: "Sijil Pencapaian",
        year2022: "Sijil Pencapaian",
        year2023: "Sijil Pencapaian",
        year2024: "Sijil Pencapaian"
      },
      esg: {
        title: "Komitmen Kami terhadap Kelestarian",
        subtitle: "Membina masa depan yang lebih hijau melalui perlombongan yang bertanggungjawab",
        environmental: {
          title: "Alam Sekitar",
          desc: "Kami komited terhadap amalan perlombongan yang mampan, termasuk pemulihan tanah yang bertanggungjawab, kawalan habuk, dan pemuliharaan air untuk meminimumkan jejak alam sekitar kami.",
          tags: ["Tindakan Iklim", "Kehidupan di Darat"]
        },
        safety: {
          title: "Keselamatan",
          desc: "Protokol keselamatan yang komprehensif dan latihan berkala memastikan kesejahteraan tenaga kerja kami dan pematuhan kepada piawaian keselamatan industri tertinggi.",
          tags: ["Kesihatan Baik", "Pekerjaan Berkualiti"]
        },
        community: {
          title: "Komuniti",
          desc: "Kami aktif melibatkan diri dengan komuniti tempatan, menyokong pembangunan ekonomi dan mewujudkan peluang pekerjaan di kawasan di mana kami beroperasi.",
          tags: ["Tiada Kemiskinan", "Pekerjaan Berkualiti", "Bandar Mampan"]
        }
      }
    },

    // ---- PRODUCTS PAGE ----
    products: {
      hero: {
        title: "Produk Kami",
        subtitle: "Granit premium dan bahan kuari untuk pembinaan dan infrastruktur"
      },
      list: {
        aggregate34: {
          name: 'AGREGAT 3/4"',
          spec: "Saiz: 20mm",
          desc: "Agregat granit hancur berkualiti tinggi untuk pengeluaran konkrit, turapan asfalt, dan pembinaan jalan raya. Diperakui SIRIM.",
          tag: "Diperakui SIRIM"
        },
        aggregate112: {
          name: 'AGREGAT 1 1/2"',
          spec: "Saiz: 40mm",
          desc: "Agregat granit premium untuk kerja konkrit berat, pembinaan berskala besar, dan projek asas.",
          tag: "Diperakui SIRIM"
        },
        msand: {
          name: "M.PASIR (DEBU KUARI)",
          spec: "Serbuk Halus",
          desc: "Pasir buatan dan debu kuari untuk pembinaan, penimbusan, plasteran, dan pengeluaran bahan binaan.",
          tag: "Kualiti Premium"
        },
        crusherRun: {
          name: 'CRUSHER RUN 2"',
          spec: "Saiz: 50mm",
          desc: "Bahan crusher run granit untuk asas jalan, sub-asas, kerja asas, dan projek pembinaan awam.",
          tag: "Gred Infrastruktur"
        },
        block: {
          name: 'BLOK 3" X 4"',
          spec: "Saiz: 75mm x 100mm",
          desc: "Blok granit dipotong mengikut saiz untuk pelapisan seni bina, landskap, dinding penahan, dan aplikasi batu hiasan.",
          tag: "Saiz Tersuai"
        },
        chipping: {
          name: 'SERPIH 3/8"',
          spec: "Saiz: 10mm",
          desc: "Serpihan granit kecil untuk landskap, laluan hiasan, sistem saliran, dan aplikasi kemasan konkrit.",
          tag: "Kualiti Premium"
        }
      }
    },

    // ---- FACILITIES PAGE ----
    facilities: {
      hero: {
        title: "Kemudahan Kami",
        subtitle: "Kemudahan kuari dan pemprosesan terkini"
      },
      list: {
        quarry: {
          title: "Kuari Granit",
          desc: "Operasi kuari utama kami merangkumi beberapa dekad pengekstrakan granit. Dilengkapi dengan teknologi penggerudian dan letupan moden."
        },
        crushing: {
          title: "Loji Penghancuran",
          desc: "Loji penghancuran dan penapisan terkini menghasilkan agregat berkualiti tinggi pelbagai saiz."
        },
        storage: {
          title: "Kemudahan Penyimpanan",
          desc: "Kawasan penyimpanan berbumbung besar untuk produk siap, memastikan bahan kekal kering dan siap untuk penghantaran."
        },
        transport: {
          title: "Fleet Pengangkutan",
          desc: "Fleet pengangkut berat dan lori kami sendiri memastikan penghantaran tepat dan boleh dipercayai di seluruh Malaysia."
        },
        safety: {
          title: "Keselamatan & Alam Sekitar",
          desc: "Komited terhadap amalan perlombongan mampan dengan pengurusan keselamatan dan alam sekitar yang komprehensif."
        }
      }
    },

    // ---- CONTACT PAGE ----
    contact: {
      hero: {
        title: "Hubungi Kami",
        subtitle: "Hubungi Teik Granite Quarry untuk semua keperluan bahan binaan anda"
      },
      form: {
        title: "Hantar Pertanyaan",
        subtitle: "Isi borang dan pasukan kami akan bertindak balas dalam masa 24 jam.",
        name: "Nama Penuh",
        namePlaceholder: "Masukkan nama penuh anda",
        nameError: "Sila masukkan nama penuh anda",
        email: "Alamat E-mel",
        emailPlaceholder: "anda@contoh.com",
        emailError: "Sila masukkan alamat e-mel yang sah",
        phone: "Nombor Telefon",
        phonePlaceholder: "012-345 6789",
        phoneError: "Sila masukkan nombor telefon anda",
        subject: "Perkara",
        subjectPlaceholder: "Pilih perkara...",
        subjectError: "Sila pilih perkara",
        subjectOptions: {
          general: "Pertanyaan Umum",
          quotation: "Sebut Harga Produk",
          bulk: "Tempahan Pukal",
          logistics: "Logistik & Penghantaran",
          partnership: "Perkongsian",
          other: "Lain-lain"
        },
        message: "Mesej",
        messagePlaceholder: "Beritahu kami tentang projek anda...",
        messageError: "Sila masukkan mesej anda",
        sending: "Menghantar...",
        send: "Hantar Mesej",
        success: {
          title: "Berjaya!",
          message: "Mesej anda telah dihantar dengan jayanya! Kami akan menghubungi anda dalam masa 24 jam."
        },
        error: {
          title: "Tidak Sah!",
          message: "Sila isi semua ruangan yang diperlukan dengan betul.",
          tryAgain: "Ada masalah. Sila cuba lagi atau hubungi kami terus melalui telefon."
        }
      },
      info: {
        headOffice: "Pejabat Utama",
        corporateOffice: "Pejabat Korporat",
        headOfficeCompany: "Teik Granite Quarry Sdn Bhd",
        corporateOfficeCompany: "Teik Granite Corporation Sdn Bhd",
        contactNumbers: "Nombor Telefon",
        companyInfo: "Maklumat Syarikat",
        email: "E-mel",
        main: "Utama",
        corporate: "Korporat"
      },
      map: {
        title: "Cari Kami",
        subtitle: "Lawati pejabat dan tapak kuari kami di seluruh Pulau Pinang",
        pulauTikus: {
          name: "Teik Granite Quarry Sdn Bhd / Teik Granite Corporation Sdn Bhd",
          location: "Pulau Tikus, Pulau Pinang",
          badge: "Pejabat Utama / Pejabat Korporat",
          address: "368-2-15, Level 2, Bellisa Row, Jalan Burmah, 10350 Pulau Pinang"
        },
        tanjungBungah: {
          name: "TGQ (Tanjung Bungah) Sdn Bhd",
          location: "Tanjung Bungah, Pulau Pinang",
          badge: "Tapak Kuari",
          address: "Tanjung Bungah, 11200 Pulau Pinang"
        },
        penanti: {
          name: "TGQ (Air Itam) Sdn Bhd",
          location: "Penanti, Pulau Pinang",
          badge: "Tapak Kuari",
          address: "Kuari Penanti, Kubang Semang, Pulau Pinang"
        }
      }
    }
  },

  // ============================================================
  // CHINESE (中文)
  // ============================================================
  zh: {
    // ---- NAVIGATION ----
    nav: {
      company: "公司",
      about: "关于",
      products: "产品",
      facilities: "设施",
      contact: "联系"
    },
    logo: {
      company: "德石较有限公司",
      reg: "196001000153 (3821-P)"
    },

    // ---- LANGUAGE NAMES ----
    lang: {
      en: "🇬🇧 EN",
      ms: "🇲🇾 BM",
      zh: "🇨🇳 CN"
    },

    // ---- COMMON ----
    common: {
      learnMore: "了解更多 →",
      requestQuote: "索取报价",
      viewMap: "在地图上查看位置",
      backToTop: "返回顶部",
      sendMessage: "发送消息",
      copyright: "© 2026 德石较有限公司. 版权所有。",
      established: "成立日期：1960年6月20日",
      regNo: "注册号：196001000153 (3821-P)",
      tagline: "值得信赖的马来西亚采石场公司，供应优质花岗岩和建筑材料超过60年。",
      quickLinks: "快速链接",
      groupCompanies: "集团公司",
      connectWithUs: "联系我们",
      phone: "04-261 4477",
      fax: "04-262 2818",
      location: "马来西亚槟城乔治市",
      watchVideo: "观看视频"
    },

    // ---- FOOTER GROUP COMPANIES ----
    footer: {
      groupCompanies: {
        teikGraniteQuarry: "德石较有限公司",
        teikGraniteCorporation: "德石较机构有限公司",
        tgqTanjungBungah: "德石较（丹绒武雅）有限公司",
        tgqAirItam: "德石较（亚依淡）有限公司"
      }
    },

    // ---- INDEX PAGE ----
    index: {
      hero: {
        badge: "自1960年",
        title: '建设马来西亚 <br /><span class="highlight-red">自1960年</span>',
        subtitle: '<strong>值得信赖的采石场基础设施与建筑解决方案</strong><br />德石较有限公司成立于1960年，拥有超过六十年的经验，供应优质花岗岩骨料、机制砂和采石材料，遍布槟城和马来西亚各地。'
      },
      overview: {
        badge: "自1960年",
        title: "公司概况",
        subtitle: "以优质花岗岩建设马来西亚基础设施",
        p1: '德石较有限公司成立于1960年，是槟城成熟的采石场运营商之一。',
        p2: "60多年来，我们为高速公路、桥梁、商业开发、住宅项目、工厂、港口和基础设施工程供应优质花岗岩产品。",
        p3: "如今，我们继续将数十年的采石经验与现代生产方式相结合，为马来西亚建筑行业提供可靠的建筑材料。",
        stats: {
          years: "卓越年数",
          companies: "集团公司",
          incorporated: "成立年份",
          location: "地点"
        }
      },
      subsidiaries: {
        title: "集团公司",
        subtitle: "德石较集团由以下实体组成",
        headOffice: "总部",
        corporateOffice: "公司办公室",
        quarrySite: "采石场",
        address: "地址",
        email: "电子邮件",
        phone: "电话",
        fax: "传真",
        website: "网站",
        site: "场地"
      },
      products: {
        title: "我们的产品",
        subtitle: "用于建筑和基础设施的优质采石材料",
        crushedStone: "碎石",
        gravel: "砾石（天然）",
        sand: "机制砂",
        recycled: "再生骨料",
        crushedStoneSpec: '3/4" & 1 1/2" & 3/8"',
        gravelSpec: "水洗与筛选",
        sandSpec: "机制砂 / 采石场粉尘",
        recycledSpec: "环保",
        crushedStoneDesc: "高品质碎石，用于混凝土生产、沥青铺设和道路建设。",
        gravelDesc: "天然砾石，用于排水系统、景观美化、人行道和装饰应用。",
        sandDesc: "机制砂，用于建筑、抹灰、回填和建筑材料生产。",
        recycledDesc: "可持续再生骨料，用于基层、道路建设和环保项目。",
        sirim: "SIRIM认证",
        premium: "优质",
        infrastructure: "基础设施级",
        eco: "环保",
        industries: {
          title: "适用行业",
          subtitle: "我们的产品广泛应用于：",
          highway: "高速公路建设",
          road: "道路工程",
          bridge: "桥梁",
          commercial: "商业建筑",
          residential: "住宅",
          industrial: "工业厂房",
          infrastructure: "基础设施项目",
          reclamation: "填海工程",
          drainage: "排水工程",
          landscaping: "园林绿化",
          readyMix: "预拌混凝土厂",
          asphalt: "沥青厂",
          civil: "土木工程项目"
        },
        showMore: "显示产品详情"
      },
      process: {
        title: "从岩石到可靠的建筑材料",
        subtitle: "我们的采石过程确保每一步的质量",
        step1: "钻孔与爆破",
        step1Desc: "控制爆破以提取优质花岗岩",
        step2: "挖掘",
        step2Desc: "液压挖掘机安全高效地清除爆破岩石",
        step3: "破碎厂",
        step3Desc: "初级和二级破碎生产不同规格的骨料",
        step4: "装载与运输",
        step4Desc: "重型自卸卡车将花岗岩运输到堆料场"
      },
      video: {
        title: "我们的采石作业",
        subtitle: "观看我们的花岗岩开采和爆破过程"
      }
    },

    // ---- ABOUT PAGE ----
    about: {
      hero: {
        title: "关于我们",
        subtitle: "自1960年以来以优质花岗岩建设马来西亚基础设施"
      },
      history: {
        title: "公司历史",
        p1: "德石较有限公司是槟城州采石行业的先驱。德石较有限公司成立于1950年代初，由家族族长 Loh Poh Heng 先生在槟城山脚下的 Air Itam 首次经营采石场。",
        p2: "在1960年代，采石业务扩展到 Tanjung Bungah，然后是岛上的 Bayan Lepas 和大陆的 Penanti, Bukit Mertajam。在高峰期，槟城有四个采石场在运营。然而，作为项目采石场运营的 Bayan Lepas 采石场仅持续了几年。Air Itam 采石场也在1992年左右自愿停止运营。",
        p3: "至今，Tanjong Bungah 和 Penanti 采石场仍然活跃，并继续作为岛上和大陆的商业采石场运营。",
        timeline: {
          title: "时间线",
          location: "采石场位置",
          status: "状态",
          airItam: "槟城 Air Itam",
          bayanLepas: "槟城 Bayan Lepas",
          tanjungBungah: "槟城 Tanjung Bungah",
          penanti: "Bukit Mertajam Penanti",
          ceased: "已停止",
          active: "运营中"
        }
      },
      projects: {
        title: "参与槟城主要项目",
        subtitle: "在运营期间，德石较有限公司作为材料供应商和承包商参与了多个主要基础设施和建筑项目",
        airport: {
          title: "槟城国际机场",
          desc: "作为分包商与 Maeda Construction Co. 合作，同时也作为所有岩石材料的供应商。",
          company: "Maeda Construction Co."
        },
        bridge: {
          title: "槟城第一大桥与林苍佑大道",
          desc: "与 Hyundai Engineering and Construction Company 合作，作为岩石材料供应商，并为林苍佑大道的一部分担任道路建设商。",
          company: "Hyundai Engineering & Construction"
        },
        komtar: {
          title: "光大大厦",
          desc: "与 Lim Kah Ngam Malaysia 合作，作为他们所有建筑工程的混凝土骨料主要供应商。",
          company: "Lim Kah Ngam Malaysia"
        },
        skyscrapers: {
          title: "槟岛主要摩天大楼",
          desc: "为槟岛各地的众多高层项目向承包商和预拌混凝土生产商提供骨料的主要供应商。",
          company: "各承包商"
        },
        northSouth: {
          title: "南北大道",
          desc: "向 United Engineers 承包商提供道路基层岩石材料和混凝土骨料，供应北部地区。",
          company: "United Engineers"
        },
        container: {
          title: "北赖集装箱码头",
          desc: "向 IJM Construction 供应护岸岩石和石块，用于港口海岸保护。",
          company: "IJM Construction"
        },
        secondBridge: {
          title: "槟城第二大桥",
          desc: "与 China Harbour Engineering Co. 合作，作为优质骨料供应商，用于他们的高等级混凝土生产。",
          company: "China Harbour Engineering Co."
        },
        doubleTrack: {
          title: "电动双轨项目",
          desc: "供应铁路道碴石以及道路基层和混凝土所需的石材材料。",
          company: "各承包商"
        }
      },
      historical: {
        title: "历史采石场遗址",
        subtitle: "保护槟城采石业的丰富遗产",
        airItam: {
          title: "Air Itam 采石场",
          subtitle: "一切开始的地方...",
          ceased: "已停止",
          caption1: "1957年 Yong Mun Sen 的绘画 - Air Itam 采石场作业",
          caption2: "带有铁路轨道的旧混凝土石 bunker 用于运输",
          text1: "著名艺术家 Yong Mun Sen 于1957年绘制的 Air Itam 采石场早期作业，当时使用风镐和人工用锤子破碎岩石是常态。",
          text2: "Air Itam 采石场的旧混凝土石 bunker。破碎成合适尺寸的石头/岩石然后通过铁路轨道上的轮式铲斗运输到 bunker 进行储存和后续销售。"
        },
        tanjungBungah: {
          title: "Tanjong Bungah 采石场",
          subtitle: "活跃的商业采石场",
          active: "运营中",
          caption1: "Tanjong Bungah 采石场区域航拍照片",
          caption2: "采石场作业鸟瞰图"
        },
        penanti: {
          title: "Penanti 采石场",
          subtitle: "活跃的商业采石场",
          active: "运营中",
          caption1: "Penanti 采石场区域航拍照片",
          caption2: "采石场作业鸟瞰图"
        }
      },
      certificate: {
        title: "证书奖项",
        subtitle: "因在采石场运营和安全标准方面的卓越表现而获得认可",
        year2021: "成就证书",
        year2022: "成就证书",
        year2023: "成就证书",
        year2024: "成就证书"
      },
      esg: {
        title: "我们对可持续发展的承诺",
        subtitle: "通过负责任的采石建设更绿色的未来",
        environmental: {
          title: "环境",
          desc: "我们致力于可持续的采石实践，包括负责任的土地复垦、粉尘控制和水资源保护，以最大限度地减少我们的环境足迹。",
          tags: ["气候行动", "陆地生态"]
        },
        safety: {
          title: "安全",
          desc: "全面的安全协议和定期培训确保我们员工的福祉，并符合最高的行业安全标准。",
          tags: ["良好健康", "体面工作"]
        },
        community: {
          title: "社区",
          desc: "我们积极参与当地社区，支持经济发展，并在我们运营的地区创造就业机会。",
          tags: ["消除贫穷", "体面工作", "可持续城市"]
        }
      }
    },

    // ---- PRODUCTS PAGE ----
    products: {
      hero: {
        title: "我们的产品",
        subtitle: "用于建筑和基础设施的优质花岗岩和采石材料"
      },
      list: {
        aggregate34: {
          name: '3/4" 骨料',
          spec: "尺寸：20mm",
          desc: "高品质碎石骨料，用于混凝土生产、沥青铺设和道路建设。SIRIM认证。",
          tag: "SIRIM认证"
        },
        aggregate112: {
          name: '1 1/2" 骨料',
          spec: "尺寸：40mm",
          desc: "优质花岗岩骨料，用于重型混凝土工程、大型建筑和基础项目。",
          tag: "SIRIM认证"
        },
        msand: {
          name: "机制砂（采石场粉尘）",
          spec: "细粉",
          desc: "机制砂和采石场粉尘，用于建筑、回填、抹灰和建筑材料生产。",
          tag: "优质"
        },
        crusherRun: {
          name: '2" 破碎跑料',
          spec: "尺寸：50mm",
          desc: "花岗岩破碎跑料，用于道路基层、底基层、基础工程和土木建筑项目。",
          tag: "基础设施级"
        },
        block: {
          name: '3" X 4" 块石',
          spec: "尺寸：75mm x 100mm",
          desc: "切割尺寸的花岗岩块石，用于建筑饰面、景观美化、挡土墙和装饰石材应用。",
          tag: "定制尺寸"
        },
        chipping: {
          name: '3/8" 碎石屑',
          spec: "尺寸：10mm",
          desc: "小颗粒花岗岩碎石屑，用于景观美化、装饰路径、排水系统和混凝土饰面应用。",
          tag: "优质"
        }
      }
    },

    // ---- FACILITIES PAGE ----
    facilities: {
      hero: {
        title: "我们的设施",
        subtitle: "最先进的采石和加工设施"
      },
      list: {
        quarry: {
          title: "花岗岩采石场",
          desc: "我们的主要采石场作业跨越数十年的花岗岩开采。配备现代钻孔和爆破技术。"
        },
        crushing: {
          title: "破碎厂",
          desc: "最先进的破碎和筛分厂，生产各种规格的高品质骨料。"
        },
        storage: {
          title: "储存设施",
          desc: "大型有顶储存区用于成品，确保材料保持干燥并准备发货。"
        },
        transport: {
          title: "运输车队",
          desc: "我们自己的重型运输车和卡车车队确保在马来西亚各地及时可靠的交付。"
        },
        safety: {
          title: "安全与环境",
          desc: "致力于可持续的采石实践，拥有全面的安全和环境管理。"
        }
      }
    },

    // ---- CONTACT PAGE ----
    contact: {
      hero: {
        title: "联系我们",
        subtitle: "联系德石较有限公司，满足您所有建筑材料需求"
      },
      form: {
        title: "发送咨询",
        subtitle: "填写表格，我们的团队将在24小时内回复。",
        name: "全名",
        namePlaceholder: "输入您的全名",
        nameError: "请输入您的全名",
        email: "电子邮件地址",
        emailPlaceholder: "you@example.com",
        emailError: "请输入有效的电子邮件地址",
        phone: "电话号码",
        phonePlaceholder: "012-345 6789",
        phoneError: "请输入您的电话号码",
        subject: "主题",
        subjectPlaceholder: "选择主题...",
        subjectError: "请选择一个主题",
        subjectOptions: {
          general: "一般咨询",
          quotation: "产品报价",
          bulk: "批量订购",
          logistics: "物流与交付",
          partnership: "合作",
          other: "其他"
        },
        message: "消息",
        messagePlaceholder: "告诉我们您的项目...",
        messageError: "请输入您的消息",
        sending: "发送中...",
        send: "发送消息",
        success: {
          title: "成功！",
          message: "您的消息已成功发送！我们将在24小时内回复您。"
        },
        error: {
          title: "无效！",
          message: "请正确填写所有必填字段。",
          tryAgain: "出了点问题。请重试或直接通过电话联系我们。"
        }
      },
      info: {
        headOffice: "总部",
        corporateOffice: "公司办公室",
        headOfficeCompany: "德石较有限公司",
        corporateOfficeCompany: "德石较机构有限公司",
        contactNumbers: "联系电话",
        companyInfo: "公司信息",
        email: "电子邮件",
        main: "总部",
        corporate: "公司"
      },
      map: {
        title: "找到我们",
        subtitle: "参观我们在槟城各地的办公室和采石场",
        pulauTikus: {
          name: "德石较有限公司 / 德石较机构有限公司",
          location: "槟城 Pulau Tikus",
          badge: "总部 / 公司办公室",
          address: "368-2-15, Level 2, Bellisa Row, Jalan Burmah, 10350 槟城"
        },
        tanjungBungah: {
          name: "德石较（丹绒武雅）有限公司",
          location: "槟城 Tanjung Bungah",
          badge: "采石场",
          address: "Tanjung Bungah, 11200 槟城"
        },
        penanti: {
          name: "德石较（亚依淡）有限公司",
          location: "槟城 Penanti",
          badge: "采石场",
          address: "Penanti 采石场, Kubang Semang, 槟城"
        }
      }
    }
  }
};

// Make it available globally
window.TRANSLATIONS = TRANSLATIONS;